...
;;-
;----------------------------------------------------------------------
;        atv3 startup, initialization, and shutdown routines
;----------------------------------------------------------------------

pro atv3_initcommon

; Routine to initialize the atv3 common blocks.  Use common blocks so
; that other IDL programs can access the atv3 internal data easily.

common atv3_state, state
common atv3_point, markcoord
common atv3_color, r_vector, g_vector, b_vector
common atv3_pdata, nplot, maxplot, plot_ptr
common atv3_images, $
  main_image, $
  display_image, $
  scaled_image, $
...
state = {                   $
          ...
          active_window_id: 0L, $        ; user's active window outside atv3
...
; This routine initializes the atv3 internal variables, and creates and
; realizes the window widgets.  It is only called by the atv3 main
; program level, when there is no previously existing atv3 window.
...
widget_control, top_menu, event_pro = 'atv3_topmenu_event'
widget_control, state.draw_widget_id, event_pro = 'atv3_draw_color_event'
widget_control, state.draw_base_id, event_pro = 'atv3_draw_base_event'
widget_control, state.keyboard_text_id, event_pro = 'atv3_keyboard_event'
widget_control, state.pan_widget_id, event_pro = 'atv3_pan_event'
...
; See the routine atv3_stretchct to see why we do it this way.
...
; Routine to tv the colorbar at the bottom of the atv3 window

common atv3_state

atv3_setwindow, state.colorbar_window_id
...
pro atv3_shutdown, windowid

; routine to kill the atv3 window(s) and clear variables to conserve
; memory when quitting atv3.  The windowid parameter is used when
; atv3_shutdown is called automatically by the xmanager, if atv3 is
...
if (xregistered ('atv3')) then widget_control, state.base_id, /destroy
...
;--------------------------------------------------------------------
;                  main atv3 event loops
;--------------------------------------------------------------------

pro atv3_topmenu_event, event
...
; of atv3 window without entering the main base
atv3_getwindow
...
; File menu options:
    'ReadFits': begin
        atv3_readfits, newimage=newimage
        if (newimage EQ 1) then begin
            atv3_getstats
            atv3_settitle
            state.zoom_level =  0
            state.zoom_factor = 1.0
            if (state.default_autoscale EQ 1) then atv3_autoscale
            atv3_set_minmax
            atv3_displayall
        endif
    end
    'WritePS' : atv3_writeps
    'WriteTiff': atv3_writetiff
    'Quit':     atv3_shutdown
; ColorMap menu options:            
    'Grayscale': atv3_getct, 0
    'Blue-White': atv3_getct, 1
    'Red-Orange': atv3_getct, 3
    'BGRY': atv3_getct, 4
...
; Label options:
    'TextLabel': atv3_textlabel
    'Contour': atv3_oplotcontour
    'Compass': atv3_setcompass
    'ScaleBar': atv3_setscalebar
    'EraseLast': atv3erase, 1
    'EraseAll': atv3erase
; Blink options:
    'SetBlink1': begin   
        atv3_setwindow, state.draw_window_id
...
; Info options:
    'Photometry': atv3_apphot
    'ImageHeader': atv3_headinfo
    'Statistics': atv3_showstats
...
; Coordinate system options:
    ...
    'RA,dec (J2000)': BEGIN 
       ...
       atv3_gettrack             ; refresh coordinate window
    END 
    'RA,dec (B1950)': BEGIN 
       state.display_coord_sys = 'RA--'
    ...
    'Native': BEGIN 
       IF (state.wcstype EQ 'angle') THEN BEGIN 
          ...
          rev = atv3_fits_ctype_reversed(astr.ctype)
...
; Need to test whether atv3 is still alive, since the quit option
; might have been selected.        
if (xregistered('atv3', /noshow)) then atv3_resetwindow

end

;--------------------------------------------------------------------

pro atv3_draw_color_event, event
...
case event.type of
    0: begin           ; button press
        if (event.press EQ 1) then begin
            state.cstretch = 1
            atv3_stretchct, event.x, event.y, /getmouse
            atv3_colorbar
        endif else begin
            atv3_zoom, 'none', /recenter
        endelse
    end
    1: begin
        state.cstretch = 0  ; button release
        if (state.bitdepth EQ 24) then atv3_refresh
        atv3_draw_motion_event, event
    end
    2: begin                ; motion event
        if (state.cstretch EQ 1) then begin
            atv3_stretchct, event.x, event.y, /getmouse 
            if (state.bitdepth EQ 24) then atv3_refresh, /fast
        endif else begin 
            atv3_draw_motion_event, event
        endelse
    end 
...
pro atv3_draw_zoom_event, event
...
if (event.type EQ 0) then begin 
    case event.press of
        1: atv3_zoom, 'in', /recenter
        2: atv3_zoom, 'none', /recenter
        4: atv3_zoom, 'out', /recenter
    endcase
endif

if (event.type EQ 2) then atv3_draw_motion_event, event
...
pro atv3_draw_blink_event, event
...
case event.type of
    ...
    2: atv3_draw_motion_event, event ; motion event
endcase
...
pro atv3_draw_phot_event, event
...
pro atv3_draw_motion_event, event
...
pro atv3_draw_base_event, event
...
case eventchar of
    '1': atv3_move_cursor, eventchar
    '2': atv3_move_cursor, eventchar
    '3': atv3_move_cursor, eventchar
    '4': atv3_move_cursor, eventchar
    '6': atv3_move_cursor, eventchar
    '7': atv3_move_cursor, eventchar
    '8': atv3_move_cursor, eventchar
    '9': atv3_move_cursor, eventchar
    'r': atv3_rowplot
    'R': atv3_rowplot, /overplot
    'c': atv3_colplot
    'C': atv3_colplot, /overplot
    's': atv3_surfplot
    'm': atv3_markpoint
    'd': atv3_displaypoint
    'k': atv3_killpoint
...
if (xregistered('atv3', /noshow)) then $
...
case event.type of
    ...
    2: begin
        atv3_pantrack, event     ; motion event
        widget_control, state.pan_widget_id, /clear_events
    end
    else:
endcase
...
pro atv3_event, event

; Main event loop for atv3 top-level base, and for all the buttons.
...
case uvalue of
    ...
    'mode': case event.index of
        0: widget_control, state.draw_widget_id, $
          event_pro = 'atv3_draw_color_event'
        1: widget_control, state.draw_widget_id, $
          event_pro = 'atv3_draw_zoom_event'
        2: widget_control, state.draw_widget_id, $
          event_pro = 'atv3_draw_blink_event'
        3: widget_control, state.draw_widget_id, $
          event_pro = 'atv3_draw_phot_event'
    ...
    'invert': begin                  ; invert the color table
        ...
        atv3_stretchct, state.brightness, state.contrast
        if (state.bitdepth EQ 24) then atv3_refresh
    end
    
    'restretch_button': atv3_restretch

    'min_text': begin     ; text entry in 'min = ' box
        atv3_get_minmax, uvalue, event.value
        atv3_displayall
    end
    ...
    'zoom_in':  atv3_zoom, 'in'         ; zoom buttons
    'zoom_out': atv3_zoom, 'out'
    'zoom_one': atv3_zoom, 'one'
...
pro atv3_message, msg_txt, msgtype=msgtype, window=window
...
;-----------------------------------------------------------------------
;      main atv3 routines for scaling, displaying, cursor tracking...
;-----------------------------------------------------------------------

pro atv3_displayall, first=first
...
; passed to atv3.  If the display image has just been moved around or
; zoomed without a change in scaling, then just call atv3_refresh
...
atv3_setwindow, state.pan_pixmap
erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
atv3_resetwindow

atv3_setwindow, state.pan_window_id
if (not(keyword_set(fast))) then erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
atv3_resetwindow
atv3_drawbox, /norefresh

if (state.bitdepth EQ 24) then atv3_colorbar

; redisplay the tracking image
if (not(keyword_set(fast))) then atv3_gettrack
...
pro atv3_move_cursor, direction
...
pro atv3_get_minmax, uvalue, newvalue
...
pro atv3_zoom, zchange, recenter = recenter
common atv3_state
...
case zchange of
    ...
    else:  print,  'problem in atv3_zoom!'
endcase
...
function atv3_fits_ctype_reversed, ctype
...
function atv3_wcsstring, lon, lat, ctype, equinox, disp_type, disp_equinox, $
            disp_base60
...
; Check for reversed (RA,dec) etc. 
  reverse = atv3_fits_ctype_reversed(ctype)
...
atv3_setwindow, state.track_window_id
tv, track_image
...
if (state.wcstype EQ 'angle') then begin
    ...
    wcsstring = atv3_wcsstring(lon, lat, (*state.astr_ptr).ctype,  $
...
pro atv3_drawbox, norefresh=norefresh
...
; (i.e. non-atv3) idl window.  Use atv3_setwindow to set the window to
; one of the atv3 window, then display something to that window, then
; use atv3_resetwindow to set the current window back to the currently
...
; graphics to the previous atv3 window actually get displayed to screen
...
pro atv3_readfits, fitsfilename=fitsfilename, newimage=newimage
...
if (n_elements(strcompress(head, /remove_all)) LT 2) then begin
    atv3_message, 'File does not appear to be a valid fits image!', $
...
if (numext EQ 0 AND naxis LT 2) then begin
   if ishealpix(npix = sxpar(head, 'NAXIS1'), /silent) EQ 0 then begin 
      atv3_message, 'Selected file is not a 2-d FITS image!', $
        window = window, msgtype = 'error'
      return
   endif
endif
...
if ((numext GT 0) AND (instrume NE 'WFPC2')) then begin
    atv3_fitsext_read, fitsfile, numext, head, cancelled
endif else if ((instrume EQ 'WFPC2') AND (naxis EQ 3)) then begin
    atv3_wfpc2_read, fitsfile, head, cancelled
endif else if ((naxis EQ 3) AND (origin EQ '2MASS')) then begin
    atv3_2mass_read, fitsfile, head, cancelled
endif else begin
    atv3_plainfits_read, fitsfile, head, cancelled
endelse
...
pro atv3_fitsext_read, fitsfile, numext, head, cancelled
...
; -------- check for healpix bintable (e.g. WMAP format)
   if ishealpix(npix = sxpar(head, 'NAXIS2'), /silent) EQ 0 then begin 
      atv3_message, 'File appears to be a FITS table, not an image.', $
...
pro atv3_plainfits_read, fitsfile, head, cancelled
...
pro atv3_wfpc2_read, fitsfile, head, cancelled
...
pro atv3_2mass_read, fitsfile, head, cancelled
...
if (filename EQ state.current_dir) then begin
    atv3_message, 'Must indicate filename to save.', msgtype = 'error', /window
    return
endif
...
fname = strcompress(state.current_dir + 'atv3.ps', /remove_all)
...
pro atv3_stretchct, brightness, contrast,  getmouse = getmouse
...
function atv3_polycolor, p

; Routine to return an vector of length !d.table_size-8,
; defined by a 5th order polynomial.   Called by atv3_makect
...
if (state.imagename EQ '') then begin
    widget_control, state.base_id, tlb_set_title = 'atv3'
endif else begin
    ...
    title = strcompress('atv3:  '+ name + '  ' + state.title_extras)
...
pro atv3_setheader, head

; Routine to keep the image header using a pointer to a 
; heap variable.  If there is no header (i.e. if atv3 has just been
...
if (xregistered('atv3_headinfo')) then begin
    widget_control, state.headinfo_base_id, /destroy
endif

if (xregistered('atv3_stats')) then begin
...
if (not(ptr_valid(state.head_ptr))) then begin
    ...
    atv3_message, 'No header information available for this image!', $
      msgtype = 'error', /window
    return
endif
...
if (not(xregistered('atv3_headinfo', /noshow))) then begin

    headinfo_base = $
      widget_base(/base_align_right, $
                  group_leader = state.base_id, $
                  /column, $
                  title = 'atv3 image header information', $
    ...
    xmanager, 'atv3_headinfo', headinfo_base, /no_block
...
pro atv3_arcbar, hdr, arclen, LABEL = label, SIZE = size, THICK = thick, $
...
; abbreviated for atv3 and modified to work with zoomed images.  For
...
for iplot = 1, nplot do begin
    case (*(plot_ptr[iplot])).type of
        'points'  : atv3_plot1plot, iplot
        'text'    : atv3_plot1text, iplot
        'contour' : atv3_plot1contour, iplot
        'compass' : atv3_plot1compass, iplot
        'scalebar': atv3_plot1scalebar, iplot
        else      : print, 'Problem in atv3_plotall!'   
    endcase
endfor
...
pro atv3plot, x, y, _extra = options
common atv3_pdata
common atv3_state
...
;  convert color names to index numbers, and set default=red
   ...
   options.color = atv3_icolor(options.color)
...
pro atv3xyouts, x, y, text, _extra = options
common atv3_pdata
common atv3_state
...
pro atv3contour, z, x, y, _extra = options
common atv3_pdata
common atv3_state
...
; need not be the same dataset displayed in the atv3 window, but it
...
;  convert color names to index numbers, and set default=red
   ...
   options.c_color = atv3_icolor(options.c_color)
...
if (NOT keyword_set(norefresh)) then atv3_refresh
...
; widget front end for atv3xyouts
...
; switch red and black indices
    ...
    atv3xyouts, textform.tag1, textform.tag2, textform.tag0, $
...
pro atv3_oplotcontour

; widget front end for atv3contour
...
; switch red and black indices
    ...
    atv3contour, main_image, c_color = labelcolor, $
...
if (nplot GE maxplot) then begin
    atv3_message, 'Total allowed number of overplots exceeded.', $
      msgtype = 'error', /window
    return
endif
...
if (state.wcstype NE 'angle') then begin 
    atv3_message, 'Cannot get coordinate info for this image!', $
      msgtype = 'error', /window
    return
endif
...
atv3_plotwindow
atv3_plot1compass, nplot

end

;---------------------------------------------------------------------

pro atv3_setscalebar
...
cform = cw_form(formdesc, /column, $
                   title = 'atv3 scalebar properties')
...
xmanager, 'atv3_lineplot', state.lineplot_base_id, /no_block
...
pro atv3_rowplot, overplot=overplot
...
pro atv3_colplot, overplot=overplot
...
if (not (xregistered('atv3_lineplot', /noshow))) then begin
    atv3_lineplot_init
endif

atv3_setwindow, state.lineplot_window_id
erase
...
atv3erase
atv3plot, markcoord[0,*], markcoord[1,*], psym=4

end

;--------------------------------------------------------------------

pro atv3_displaypoint
...
i = i + 1
h[i] =  'To pass an array to atv3:'
i = i + 1
h[i] =  '   atv3, array_name [, options]'
i = i + 1
h[i] = 'To pass a fits filename to atv3:'
i = i + 1
h[i] = '    atv3, fitsfile_name [, options] (enclose filename in single quotes) '
i = i + 1
...
i = i + 1
h[i] = '    atv3contour, array_name [, options...]'
i = i + 1
h[i] = 'To overplot text on the draw window: '
i = i + 1
h[i] = '    atv3xyouts, x, y, text_string [, options]  (enclose string in single quotes)'
i = i + 1
h[i] = 'To overplot points or lines on the current plot:'
i = i + 1
h[i] = '    atv3plot, xvector, yvector [, options]'
i = i + 2
h[i] = 'The options for atv3contour, atv3xyouts, and atv3plot are essentially'
i = i + 1
...
h[i] = 'Other commands:'
i = i + 1
h[i] = 'atv3erase [, N]:       erases all (or last N) plots and text'
i = i + 1
h[i] = 'atv3_shutdown:   quits atv3'
i = i + 1
...
i = i + 1
h[i] = 'http://www.astro.caltech.edu/~barth/atv3/atv3.html'


if (not (xregistered('atv3_help', /noshow))) then begin

helptitle = strcompress('atv3 v' + state.version + ' help')
    ...
    xmanager, 'atv3_help', help_base, /no_block
...
if (not (xregistered('atv3_stats', /noshow))) then begin

    stats_base = $
      widget_base(group_leader = state.base_id, $
                  /column, $
                  /base_align_center, $
                  title = 'atv3 image statistics', $
                  uvalue = 'stats_base')
    ...
    xmanager, 'atv3_stats', stats_base, /no_block
...
atv3_setwindow, state.statzoom_window_id
erase
...
function atv3_splinefwhm, rad, prof, splrad, splprof
...
fwhm = atv3_splinefwhm(out[*,0],out[*,6])
...
; Run atv3_radplotf and plot the results

atv3_setwindow, state.radplot_window_id
atv3_radplotf, x, y, fwhm
...
; Uncomment next lines if you want atv3 to output the WCS coords of 
; the centroid for the photometry object:
;if (state.wcstype EQ 'angle') then begin
;    xy2ad, state.centerpos[0], state.centerpos[1], *(state.astr_ptr), $
;      clon, clat
;    wcsstring = atv3_wcsstring(clon, clat, (*state.astr_ptr).ctype,  $
...
atv3_setwindow, state.photzoom_window_id
erase
...
case uvalue of
    ...
    'photsettings': atv3_apphot_settings
...
textform = cw_form(formdesc, /column, $
                   title = 'atv3 photometry settings')
...
if (not (xregistered('atv3_apphot', /noshow))) then begin

    apphot_base = $
      widget_base(/base_align_center, $
                  group_leader = state.base_id, $
                  /column, $
                  title = 'atv3 aperture photometry', $
                  uvalue = 'apphot_base')
    ...
    xmanager, 'atv3_apphot', apphot_base, /no_block
...
if ( (n_params() EQ 0) AND (xregistered('atv3', /noshow))) then begin
    print, 'USAGE: atv3, array_name OR fitsfile'
...
; Before starting up atv3, get the user's external window id.  We can't
; use the atv3_getwindow routine yet because we haven't run atv3
; startup.  A subtle issue: atv3_resetwindow won't work the first time
...
if ( (n_params() NE 0) AND (imtype EQ 'STRING')) then begin
    ...
    if (count EQ 0) then begin
        ...
        atv3_readfits, fitsfilename=image, newimage=newimage
    endelse
endif
...
; If user has passed atv3 a data array, read it into main_image.
if ( (n_params() NE 0) AND (imtype NE 'STRING') AND $
...
if (not(xregistered('atv3', /noshow))) then begin
    nb = abs(block - 1)
    xmanager, 'atv3', state.base_id, no_block = nb, cleanup = 'atv3_shutdown'
    wset, userwindow
    ; if blocking mode is set, then when the procedure reaches this
    ; line atv3 has already been terminated.  If non-blocking, then
