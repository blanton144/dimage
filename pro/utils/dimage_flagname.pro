...
;-
;------------------------------------------------------------------------------
function dimage_flagname, flagprefix, flagvalue, concat=concat, silent=silent

   ; Declare a common block so that the mask names are remembered between calls.
   common com_dimage_maskbits, maskbits

   if (n_params() NE 2 OR n_elements(flagprefix) NE 1) then begin
      print, 'Syntax - label = dimage_flagname(flagprefix, flagvalue, [ /concat ] )'
      return, ''
   endif
   ...
   if (NOT keyword_set(maskbits)) then begin
      maskfile = filepath('dimage_maskbits.par', $
       root_dir=getenv('DIMAGE_DIR'), subdirectory='data')
