...
;;-
;----------------------------------------------------------------------
;        atv2 startup, initialization, and shutdown routines
;----------------------------------------------------------------------

pro atv2_initcommon

; Routine to initialize the atv2 common blocks.  Use common blocks so
; that other IDL programs can access the atv2 internal data easily.

common atv2_state, state
common atv2_point, markcoord
common atv2_color, r_vector, g_vector, b_vector
common atv2_pdata, nplot, maxplot, plot_ptr
common atv2_images, $
  main_image, $
  display_image, $
  scaled_image, $
...
state = {                   $
          ...
          active_window_id: 0L, $        ; user's active window outside atv2
...
; This routine initializes the atv2 internal variables, and creates and
; realizes the window widgets.  It is only called by the atv2 main
; program level, when there is no previously existing atv2 window.
...
widget_control, top_menu, event_pro = 'atv2_topmenu_event'
widget_control, state.draw_widget_id, event_pro = 'atv2_draw_color_event'
widget_control, state.draw_base_id, event_pro = 'atv2_draw_base_event'
widget_control, state.keyboard_text_id, event_pro = 'atv2_keyboard_event'
widget_control, state.pan_widget_id, event_pro = 'atv2_pan_event'
...
; See the routine atv2_stretchct to see why we do it this way.
...
; Routine to tv the colorbar at the bottom of the atv2 window

common atv2_state

atv2_setwindow, state.colorbar_window_id
...
pro atv2_shutdown, windowid

; routine to kill the atv2 window(s) and clear variables to conserve
; memory when quitting atv2.  The windowid parameter is used when
; atv2_shutdown is called automatically by the xmanager, if atv2 is
...
if (xregistered ('atv2')) then widget_control, state.base_id, /destroy
...
;--------------------------------------------------------------------
;                  main atv2 event loops
;--------------------------------------------------------------------

pro atv2_topmenu_event, event
...
; of atv2 window without entering the main base
atv2_getwindow
...
; File menu options:
    'ReadFits': begin
        atv2_readfits, newimage=newimage
        if (newimage EQ 1) then begin
            atv2_getstats
            atv2_settitle
            state.zoom_level =  0
            state.zoom_factor = 1.0
            if (state.default_autoscale EQ 1) then atv2_autoscale
            atv2_set_minmax
            atv2_displayall
        endif
    end
    'WritePS' : atv2_writeps
    'WriteTiff': atv2_writetiff
    'Quit':     atv2_shutdown
; ColorMap menu options:            
    'Grayscale': atv2_getct, 0
    'Blue-White': atv2_getct, 1
    'Red-Orange': atv2_getct, 3
    'BGRY': atv2_getct, 4
    'Rainbow': atv2_getct, 13
    'Stern Special': atv2_getct, 15
    'Green-White': atv2_getct, 8
    'ATV2 Special': atv2_makect, event_name
; Scaling options:
    'Linear': begin
        state.scaling = 0
        atv2_displayall
...
; Label options:
    'TextLabel': atv2_textlabel
    'Contour': atv2_oplotcontour
    'Compass': atv2_setcompass
    'ScaleBar': atv2_setscalebar
    'EraseLast': atv2erase, 1
    'EraseAll': atv2erase
; Blink options:
    'SetBlink1': begin   
        atv2_setwindow, state.draw_window_id
...
; Info options:
    'Photometry': atv2_apphot
    'ImageHeader': atv2_headinfo
    'Statistics': atv2_showstats
...
; Coordinate system options:
    ...
    'RA,dec (J2000)': BEGIN 
       ...
       atv2_gettrack             ; refresh coordinate window
    END 
    'RA,dec (B1950)': BEGIN 
       state.display_coord_sys = 'RA--'
    ...
    'Native': BEGIN 
       IF (state.wcstype EQ 'angle') THEN BEGIN 
          ...
          rev = atv2_fits_ctype_reversed(astr.ctype)
...
; Need to test whether atv2 is still alive, since the quit option
; might have been selected.        
if (xregistered('atv2', /noshow)) then atv2_resetwindow

end

;--------------------------------------------------------------------

pro atv2_draw_color_event, event
...
case event.type of
    0: begin           ; button press
        if (event.press EQ 1) then begin
            state.cstretch = 1
            atv2_stretchct, event.x, event.y, /getmouse
            atv2_colorbar
        endif else begin
            atv2_zoom, 'none', /recenter
        endelse
    end
    1: begin
        state.cstretch = 0  ; button release
        if (state.bitdepth EQ 24) then atv2_refresh
        atv2_draw_motion_event, event
    end
    2: begin                ; motion event
        if (state.cstretch EQ 1) then begin
            atv2_stretchct, event.x, event.y, /getmouse 
            if (state.bitdepth EQ 24) then atv2_refresh, /fast
        endif else begin 
            atv2_draw_motion_event, event
        endelse
    end 
...
pro atv2_draw_zoom_event, event
...
if (event.type EQ 0) then begin 
    case event.press of
        1: atv2_zoom, 'in', /recenter
        2: atv2_zoom, 'none', /recenter
        4: atv2_zoom, 'out', /recenter
    endcase
endif

if (event.type EQ 2) then atv2_draw_motion_event, event
...
pro atv2_draw_blink_event, event
...
case event.type of
    ...
    2: atv2_draw_motion_event, event ; motion event
endcase
...
pro atv2_draw_phot_event, event
...
pro atv2_draw_motion_event, event
...
pro atv2_draw_base_event, event
...
case eventchar of
    '1': atv2_move_cursor, eventchar
    '2': atv2_move_cursor, eventchar
    '3': atv2_move_cursor, eventchar
    '4': atv2_move_cursor, eventchar
    '6': atv2_move_cursor, eventchar
    '7': atv2_move_cursor, eventchar
    '8': atv2_move_cursor, eventchar
    '9': atv2_move_cursor, eventchar
    'r': atv2_rowplot
    'R': atv2_rowplot, /overplot
    'c': atv2_colplot
    'C': atv2_colplot, /overplot
    's': atv2_surfplot
    'm': atv2_markpoint
    'd': atv2_displaypoint
    'k': atv2_killpoint
...
if (xregistered('atv2', /noshow)) then $
...
case event.type of
    ...
    2: begin
        atv2_pantrack, event     ; motion event
        widget_control, state.pan_widget_id, /clear_events
    end
    else:
endcase
...
pro atv2_event, event

; Main event loop for atv2 top-level base, and for all the buttons.
...
case uvalue of
    ...
    'mode': case event.index of
        0: widget_control, state.draw_widget_id, $
          event_pro = 'atv2_draw_color_event'
        1: widget_control, state.draw_widget_id, $
          event_pro = 'atv2_draw_zoom_event'
        2: widget_control, state.draw_widget_id, $
          event_pro = 'atv2_draw_blink_event'
        3: widget_control, state.draw_widget_id, $
          event_pro = 'atv2_draw_phot_event'
    ...
    'invert': begin                  ; invert the color table
        ...
        atv2_stretchct, state.brightness, state.contrast
        if (state.bitdepth EQ 24) then atv2_refresh
    end
    
    'restretch_button': atv2_restretch

    'min_text': begin     ; text entry in 'min = ' box
        atv2_get_minmax, uvalue, event.value
        atv2_displayall
    end
    ...
    'zoom_in':  atv2_zoom, 'in'         ; zoom buttons
    'zoom_out': atv2_zoom, 'out'
    'zoom_one': atv2_zoom, 'one'
...
pro atv2_message, msg_txt, msgtype=msgtype, window=window
...
;-----------------------------------------------------------------------
;      main atv2 routines for scaling, displaying, cursor tracking...
;-----------------------------------------------------------------------

pro atv2_displayall, first=first
...
; passed to atv2.  If the display image has just been moved around or
; zoomed without a change in scaling, then just call atv2_refresh
...
atv2_setwindow, state.pan_pixmap
erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
atv2_resetwindow

atv2_setwindow, state.pan_window_id
if (not(keyword_set(fast))) then erase
tv, pan_image, state.pan_offset[0], state.pan_offset[1]
atv2_resetwindow
atv2_drawbox, /norefresh

if (state.bitdepth EQ 24) then atv2_colorbar

; redisplay the tracking image
if (not(keyword_set(fast))) then atv2_gettrack
...
pro atv2_move_cursor, direction
...
pro atv2_get_minmax, uvalue, newvalue
...
pro atv2_zoom, zchange, recenter = recenter
common atv2_state
...
case zchange of
    ...
    else:  print,  'problem in atv2_zoom!'
endcase
...
function atv2_fits_ctype_reversed, ctype
...
function atv2_wcsstring, lon, lat, ctype, equinox, disp_type, disp_equinox, $
            disp_base60
...
; Check for reversed (RA,dec) etc. 
  reverse = atv2_fits_ctype_reversed(ctype)
...
atv2_setwindow, state.track_window_id
tv, track_image
...
if (state.wcstype EQ 'angle') then begin
    ...
    wcsstring = atv2_wcsstring(lon, lat, (*state.astr_ptr).ctype,  $
...
pro atv2_drawbox, norefresh=norefresh
...
; (i.e. non-atv2) idl window.  Use atv2_setwindow to set the window to
; one of the atv2 window, then display something to that window, then
; use atv2_resetwindow to set the current window back to the currently
...
; graphics to the previous atv2 window actually get displayed to screen
...
pro atv2_readfits, fitsfilename=fitsfilename, newimage=newimage
...
if (n_elements(strcompress(head, /remove_all)) LT 2) then begin
    atv2_message, 'File does not appear to be a valid fits image!', $
...
if (numext EQ 0 AND naxis LT 2) then begin
   if ishealpix(npix = sxpar(head, 'NAXIS1'), /silent) EQ 0 then begin 
      atv2_message, 'Selected file is not a 2-d FITS image!', $
        window = window, msgtype = 'error'
      return
   endif
endif
...
if ((numext GT 0) AND (instrume NE 'WFPC2')) then begin
    atv2_fitsext_read, fitsfile, numext, head, cancelled
endif else if ((instrume EQ 'WFPC2') AND (naxis EQ 3)) then begin
    atv2_wfpc2_read, fitsfile, head, cancelled
endif else if ((naxis EQ 3) AND (origin EQ '2MASS')) then begin
    atv2_2mass_read, fitsfile, head, cancelled
endif else begin
    atv2_plainfits_read, fitsfile, head, cancelled
endelse
...
pro atv2_fitsext_read, fitsfile, numext, head, cancelled
...
; -------- check for healpix bintable (e.g. WMAP format)
   if ishealpix(npix = sxpar(head, 'NAXIS2'), /silent) EQ 0 then begin 
      atv2_message, 'File appears to be a FITS table, not an image.', $
...
pro atv2_plainfits_read, fitsfile, head, cancelled
...
pro atv2_wfpc2_read, fitsfile, head, cancelled
...
pro atv2_2mass_read, fitsfile, head, cancelled
...
if (filename EQ state.current_dir) then begin
    atv2_message, 'Must indicate filename to save.', msgtype = 'error', /window
    return
endif
...
fname = strcompress(state.current_dir + 'atv2.ps', /remove_all)
...
pro atv2_stretchct, brightness, contrast,  getmouse = getmouse
...
function atv2_polycolor, p

; Routine to return an vector of length !d.table_size-8,
; defined by a 5th order polynomial.   Called by atv2_makect
...
if (state.imagename EQ '') then begin
    widget_control, state.base_id, tlb_set_title = 'atv2'
endif else begin
    ...
    title = strcompress('atv2:  '+ name + '  ' + state.title_extras)
...
pro atv2_setheader, head

; Routine to keep the image header using a pointer to a 
; heap variable.  If there is no header (i.e. if atv2 has just been
...
if (xregistered('atv2_headinfo')) then begin
    widget_control, state.headinfo_base_id, /destroy
endif

if (xregistered('atv2_stats')) then begin
...
if (not(ptr_valid(state.head_ptr))) then begin
    ...
    atv2_message, 'No header information available for this image!', $
      msgtype = 'error', /window
    return
endif
...
if (not(xregistered('atv2_headinfo', /noshow))) then begin

    headinfo_base = $
      widget_base(/base_align_right, $
                  group_leader = state.base_id, $
                  /column, $
                  title = 'atv2 image header information', $
    ...
    xmanager, 'atv2_headinfo', headinfo_base, /no_block
...
pro atv2_arcbar, hdr, arclen, LABEL = label, SIZE = size, THICK = thick, $
...
; abbreviated for atv2 and modified to work with zoomed images.  For
...
for iplot = 1, nplot do begin
    case (*(plot_ptr[iplot])).type of
        'points'  : atv2_plot1plot, iplot
        'text'    : atv2_plot1text, iplot
        'contour' : atv2_plot1contour, iplot
        'compass' : atv2_plot1compass, iplot
        'scalebar': atv2_plot1scalebar, iplot
        else      : print, 'Problem in atv2_plotall!'   
    endcase
endfor
...
pro atv2plot, x, y, _extra = options
common atv2_pdata
common atv2_state
...
if (not(xregistered('atv2', /noshow))) then begin
...
;  convert color names to index numbers, and set default=red
   ...
   options.color = atv2_icolor(options.color)
...
pro atv2xyouts, x, y, text, _extra = options
common atv2_pdata
common atv2_state
...
pro atv2contour, z, x, y, _extra = options
common atv2_pdata
common atv2_state
...
; need not be the same dataset displayed in the atv2 window, but it
...
;  convert color names to index numbers, and set default=red
   ...
   options.c_color = atv2_icolor(options.c_color)
...
pro atv2erase, nerase, norefresh = norefresh
common atv2_pdata
...
if (NOT keyword_set(norefresh)) then atv2_refresh
...
; widget front end for atv2xyouts
...
; switch red and black indices
    ...
    atv2xyouts, textform.tag1, textform.tag2, textform.tag0, $
...
pro atv2_oplotcontour

; widget front end for atv2contour
...
; switch red and black indices
    ...
    atv2contour, main_image, c_color = labelcolor, $
...
if (nplot GE maxplot) then begin
    atv2_message, 'Total allowed number of overplots exceeded.', $
      msgtype = 'error', /window
    return
endif
...
if (state.wcstype NE 'angle') then begin 
    atv2_message, 'Cannot get coordinate info for this image!', $
      msgtype = 'error', /window
    return
endif
...
atv2_plotwindow
atv2_plot1compass, nplot

end

;---------------------------------------------------------------------

pro atv2_setscalebar
...
cform = cw_form(formdesc, /column, $
                   title = 'atv2 scalebar properties')
...
xmanager, 'atv2_lineplot', state.lineplot_base_id, /no_block
...
pro atv2_rowplot, overplot=overplot
...
pro atv2_colplot, overplot=overplot
...
if (not (xregistered('atv2_lineplot', /noshow))) then begin
    atv2_lineplot_init
endif

atv2_setwindow, state.lineplot_window_id
erase
...
atv2erase
atv2plot, markcoord[0,*], markcoord[1,*], psym=4

end

;--------------------------------------------------------------------

pro atv2_displaypoint
...
i = i + 1
h[i] =  'To pass an array to atv2:'
i = i + 1
h[i] =  '   atv2, array_name [, options]'
i = i + 1
h[i] = 'To pass a fits filename to atv2:'
i = i + 1
h[i] = '    atv2, fitsfile_name [, options] (enclose filename in single quotes) '
i = i + 1
...
i = i + 1
h[i] = '    atv2contour, array_name [, options...]'
i = i + 1
h[i] = 'To overplot text on the draw window: '
i = i + 1
h[i] = '    atv2xyouts, x, y, text_string [, options]  (enclose string in single quotes)'
i = i + 1
h[i] = 'To overplot points or lines on the current plot:'
i = i + 1
h[i] = '    atv2plot, xvector, yvector [, options]'
i = i + 2
h[i] = 'The options for atv2contour, atv2xyouts, and atv2plot are essentially'
i = i + 1
...
h[i] = 'Other commands:'
i = i + 1
h[i] = 'atv2erase [, N]:       erases all (or last N) plots and text'
i = i + 1
h[i] = 'atv2_shutdown:   quits atv2'
i = i + 1
h[i] = 'NOTE: If atv2 should crash, type atv2_shutdown at the idl prompt.'
i = i + 5
h[i] = strcompress('ATV2.PRO version '+state.version)
i = i + 1
h[i] = 'For full instructions, or to download the most recent version, go to:'
i = i + 1
h[i] = 'http://www.astro.caltech.edu/~barth/atv2/atv2.html'


if (not (xregistered('atv2_help', /noshow))) then begin

helptitle = strcompress('atv2 v' + state.version + ' help')
    ...
    xmanager, 'atv2_help', help_base, /no_block
...
if (not (xregistered('atv2_stats', /noshow))) then begin

    stats_base = $
      widget_base(group_leader = state.base_id, $
                  /column, $
                  /base_align_center, $
                  title = 'atv2 image statistics', $
                  uvalue = 'stats_base')
    ...
    xmanager, 'atv2_stats', stats_base, /no_block
...
atv2_setwindow, state.statzoom_window_id
erase
...
function atv2_splinefwhm, rad, prof, splrad, splprof
...
fwhm = atv2_splinefwhm(out[*,0],out[*,6])
...
; Run atv2_radplotf and plot the results

atv2_setwindow, state.radplot_window_id
atv2_radplotf, x, y, fwhm
...
; Uncomment next lines if you want atv2 to output the WCS coords of 
; the centroid for the photometry object:
;if (state.wcstype EQ 'angle') then begin
;    xy2ad, state.centerpos[0], state.centerpos[1], *(state.astr_ptr), $
;      clon, clat
;    wcsstring = atv2_wcsstring(clon, clat, (*state.astr_ptr).ctype,  $
...
atv2_setwindow, state.photzoom_window_id
erase
...
case uvalue of
    ...
    'photsettings': atv2_apphot_settings
...
textform = cw_form(formdesc, /column, $
                   title = 'atv2 photometry settings')
...
if (not (xregistered('atv2_apphot', /noshow))) then begin

    apphot_base = $
      widget_base(/base_align_center, $
                  group_leader = state.base_id, $
                  /column, $
                  title = 'atv2 aperture photometry', $
                  uvalue = 'apphot_base')
    ...
    xmanager, 'atv2_apphot', apphot_base, /no_block
...
;--------------------------------------------------------------------
;    atv2 main program.  needs to be last in order to compile.
;---------------------------------------------------------------------

; Main program routine for ATV2.  If there is no current ATV2 session,
; then run atv2_startup to create the widgets.  If ATV2 already exists,
...
if ( (n_params() EQ 0) AND (xregistered('atv2', /noshow))) then begin
    print, 'USAGE: atv2, array_name OR fitsfile'
...
; Before starting up atv2, get the user's external window id.  We can't
; use the atv2_getwindow routine yet because we haven't run atv2
; startup.  A subtle issue: atv2_resetwindow won't work the first time
...
if ( (n_params() NE 0) AND (imtype EQ 'STRING')) then begin
    ...
    if (count EQ 0) then begin
        ...
        atv2_readfits, fitsfilename=image, newimage=newimage
    endelse
endif
...
; If user has passed atv2 a data array, read it into main_image.
if ( (n_params() NE 0) AND (imtype NE 'STRING') AND $
...
if (not(xregistered('atv2', /noshow))) then begin
    nb = abs(block - 1)
    xmanager, 'atv2', state.base_id, no_block = nb, cleanup = 'atv2_shutdown'
    wset, userwindow
    ; if blocking mode is set, then when the procedure reaches this
    ; line atv2 has already been terminated.  If non-blocking, then
