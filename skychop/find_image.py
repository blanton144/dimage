...
for i in range(len(closestCenters)):
	if closestCenters[i] == 0: pass
		...
		for letter in bands:		
			ic.gunzipIt("%s-%s.fits.gz" % (fileName, letter), fileDir, outDir)
...
if np.shape(allFileNamesT)[1] == 1:
	...
	for k in range(np.shape(allFileNamesT)[0]):
		...
		swarpKARGS = "-IMAGEOUT_NAME=" + coaddFname + " -VERBOSE_TYPE=FULL -RESAMPLE_DIR=sdss-tmp -WEIGHTOUT_NAME=sdss-tmp/weight.fits -DELETE_TMPFILES=N"
