...
for i in range(len(closestCenters)):
	if closestCenters[i] == 0: pass
		...
		rectCenter, rectSize = ic.cutSection(targetImgCorners[i], oppositeImgCorners[i], \
			closestCenters[i],(RADeg/ np.cos(decDeg * pi / 180.0),decDeg), (xSize, ySize), tableData)						# For each subsection of the target image, find the center,x size,y size to give to clipfits
