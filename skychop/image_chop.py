...
def cutSection(tgCnr, opCnr, mosCen, tgCen, size):
	...
	XDs = [fabs(tgCnr[0]/ np.cos((tgCnr[1])*pi/180.0)-(mosCen[0] + ALPHA/2.0)/ np.cos((mosCen[1] + BETA/2.0)*pi/180.0), \
		   fabs(tgCnr[0]/ np.cos((tgCnr[1])*pi/180.0)-(tgCen[0] + (ALPHA*size[0]/2.0))/ np.cos((tgCen[1] + (BETA*size[1]/2.0)*pi/180.0))]
