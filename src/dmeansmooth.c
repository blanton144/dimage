...
int dmeansmooth(float *image, 
								...
								float *smooth)
{
  int i,j,k,ip,jp,ist,jst,nxt,nyt,nb,ind,jnd,sp;
  int xoff, yoff, nxgrid, nygrid;
  int ypsize, ymsize, xpsize, xmsize;
  float dx,dy,xkernel,ykernel, maxarr, minarr, total;
  ...
  for(j=0;j<nygrid;j++) {
    ...
    for(i=0;i<nxgrid;i++) {
      ...
      if(nb>1 && maxarr!=minarr) {
				...
				for(k=0;k<nb;k++)
					total+=arr[k];
        grid[i+j*nxgrid]=total/((float) nb);
			} else {
