"""
Simple determination of positions of stars in an image
...
def simplexy(image, psf_sigma=1., plim=8., dlim=1., saddle=3., maxper=1000,
    ...
    ----------
    image : 2-D ndarray 
    psf_sigma : sigma of Gaussian PSF to assume (default 1 pixel)
    plim : significance to select objects on (default 8)
    dlim : tolerance for closeness of pairs of objects (default 1 pixel)
    saddle : tolerance for depth of saddle point to separate sources 
             (default 3 sigma)
    maxper : maximum number of children per parent (default 1000)
    maxnpeaks : maximum number of stars to find total (default 100000)

    Returns
    -------
    (x, y, flux) : tuple of ndarrays with pixel positions and peak
                   pixel values of stars
